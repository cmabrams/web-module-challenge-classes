...
class Car {
  ...
  drive(distance){
    let tankMiles = this.tank*this.milesPerGallon;
    let milesString = '';
    if(tankMiles<=distance){
      this.tank = 0;
      this.odometer = this.odometer + tankMiles;
      milesString = `I ran out of fuel at ${this.odometer} miles!`; 
    } else{
      this.odometer = this.odometer + distance;
      this.tank = this.tank - distance/this.milesPerGallon;
      milesString = 'I drove successfully.'
    }
    return milesString;
  }

}
const honda = new Car('civic',20);
...
class Student extends Lambdasian{
   ...
   listSubjects(){
    let strFavSubject = 'Loving ';
    for(let i=0;i<this.favSubjects.length;i++) {
      strFavSubject += this.favSubjects[i] + ', ';
    }
...
const penelope = new Student({
  name: 'Penelope',
  age:'2',
  location: 'Weston',
  previousBackground: 'baby',
  className:'CS132',
  favSubjects: ['Baby Shark','Baby Anabelle','myGym']
})

console.log(penelope.listSubjects());
